...
# Introduction
Before running the Airflow DAG we need to create a MS SQL server and prepare there databases, schemas and tables with data which dbt will use for performing data transformations. 

It will be created using the 'mssql' service in the `docker-compose.yml` and the `dockerfiles/mssql/Dockerfile.mssql` file.
...
We need to provide there a password which will be used for logging into the created MS SQL server. We assign that password to the `SA_PASSWORD` environment variable in the Dockerfile.
