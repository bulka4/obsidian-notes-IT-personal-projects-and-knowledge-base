...
# Introduction
In a sequential consistency model ([[Backend Engineering - Distributed microservices - Consistency models|link]]), when one service makes a write (changes state), all services observe writes in the same order, but not necessarily immediately. 
