...
# Introduction
Vector clock is an example of a logical clock ([[Backend Engineering - Distributed microservices - Logical clocks (timestamps)|link]]). It is similar to Lamport clocks ([[Backend Engineering - Distributed microservices - Lamport logical clock|link]]), it is also a logical clock counter but each node stores a vector of timestamps from every node:
```
[A=5, B=2, C=9]
```
