...
# Introduction
Backpropagation is used to calculate efficiently derivatives in neural network needed to use an optimizer (e.g. gradient descent ([[Gradient Descent - ML|link]]) or Adam ([[Adam optimizer|link]])) to train it (update its parameters to reduce loss).
