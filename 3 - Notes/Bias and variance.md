...
# Introduction
**Bias** measures how far are model's predictions from the true value on average for a given data sample when training a model many times on different datasets.
...
Those are only conceptual values, we can't calculate them because they describe how model would behave on all the possible datasets which we don't have access to.

We can approximate them by training a model on different datasets, making predictions and comparing them with real values.
