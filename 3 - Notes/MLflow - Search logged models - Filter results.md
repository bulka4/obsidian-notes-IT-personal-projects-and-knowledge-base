...
# Introduction
When searching through logged models using the `search_logged_models` function in MLflow, we can filter results.

For example, to find models with a specific metric values:
```python
runs = mlflow.search_logged_models(
