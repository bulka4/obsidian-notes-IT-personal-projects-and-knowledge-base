...
Before we start using this code we need to satisfy prerequisites described here - [[MLflow on AKS project - Prerequisites|link]].
## Creating Azure resources and generating a Dockerfile
Use Terraform to create Azure resources and generate Dockerfile and `values.yaml` files ([[MLflow on AKS project - Creating Azure resources with Terraform|link]], [[MLflow on AKS project - Files generated by Terraform (using template files)|link]]):
```
# Run "terraform init" only when running Terraform for the first time in this 
# repository
terraform init
...
More information about what resources we deploy can be found here - [[MLflow on AKS project - MLflow setup Helm chart|link]].
...
```
2025/08/30 11:08 INFO mlflow.projects: == RUN (ID 'asfkj4594358439dfgkldfg87') succeeded ==
View run test at: http://mlflow-service.mlflow.svc:5000/#experiments/0/runs/asfkj4594358439dfgkldfg87
View experiment at http://mlflow-service.mlflow.svc:5000/#experiments/0
```

Also on the MLflow Tracking Server website we should see our run:  
![[2 - Images/MLflow on AKS project/screenshot 1.png]]
