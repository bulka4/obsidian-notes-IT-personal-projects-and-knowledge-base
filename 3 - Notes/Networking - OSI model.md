...
- Addressing
	- It uses IP addresses to identify source and destination devices on the network.
- Routing
	- Determines the best path for data to travel from source to destination
- Packet Forwarding
	- Transfers packets from one network segment to another using routers
...
- **Segmentation and reassemly**
	- Breaks big packets of data into smaller pieces at a sender and reassemblies them at a receiver.
- **Connection Management**
	- **Connection-oriented communication** (e.g., TCP): Establishes, maintains, and terminates a connection.
	- **Connectionless communication** (e.g., UDP): No session is maintained between devices
